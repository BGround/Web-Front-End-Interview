...
#### 6. 变量提升和函数声明提前
V8 对于提升函数和提升变量的策略是不同的，如果提升了一个变量，那么 V8 在将变量提升到作用域中时，还会为其设置默认值 undefined;
...
*简单点说就是，**浅拷贝**之后改变一个数据对象，另一个也同样改变，而**深拷贝**之后，两个数据对象互不关联*
...
一般来说我们是不应该能够获取到这个值的，但是现在浏览器中都实现了 __proto__ 属性来让我们访问这个属性，但是我们最好不要使用这个属性，因为它不是规范中规定的。
...
**拓展:** 如何阻止事件冒泡？
在标准事件浏览器中，通过e.stopPropagation()方法来阻止事件冒泡
